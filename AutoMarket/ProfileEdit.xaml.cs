...
public partial class ProfileEdit : ContentPage
{
    ...
    private async Task LoadUserProfile()
    {
        ...
        if (error != null)
        {
            await DisplayAlert("Помилка завантаження", error, "OK");
            return;
        }

        if (profile == null) return;

        // 🔥 2. Гарантуємо, що UI оновлюється в головному потоці
        // Це рятує від крашів, якщо API повернув відповідь у фоновому потоці
        MainThread.BeginInvokeOnMainThread(() =>
        {
            try
            {
                ...
                // 👇 ВИПРАВЛЕННЯ ЛОГІКИ (зберігаємо email)
            ...
            {
                // Якщо UI впаде при малюванні - ми це побачимо, але програма не закриється
